...
def get_avg_s_test(model, dataloader, random_sampler, constraint, weights, r, _dataset, _seed, _sen_attr, recursion_depth, option, save=True):
    avg_s_test(model=model, dataloader=dataloader, random_sampler=random_sampler, constraint=constraint, weights=weights, r=r, recursion_depth=recursion_depth, _dataset=_dataset, _seed=_seed, _sen_attr=_sen_attr, save=save, option=option)

def get_influence_score(model, dataloader, s_test_dataloader, random_sampler, constraint, weights, _dataset, _seed, _sen_attr, recursion_depth, r, option, load_s_test=True):
    influences = calc_influence_dataset(model=model, dataloader=dataloader, s_test_dataloader = s_test_dataloader, random_sampler=random_sampler, constraint=constraint, weights=weights, recursion_depth=recursion_depth, r=r, _dataset=_dataset, _seed=_seed, _sen_attr=_sen_attr, load_s_test=load_s_test, option=option)

    
    if option == 'fair':
        with open("./influence_score/{}_influence_score_seed_{}_sen_attr_{}.txt".format(_dataset, _seed, _sen_attr), "wb") as fp:
            pickle.dump(influences, fp)
    elif option == 'val_loss':
        with open("./influence_score/{}_val_loss_influence_score_seed_{}_sen_attr_{}.txt".format(_dataset, _seed, _sen_attr), "wb") as fp:
            pickle.dump(influences, fp)
...
if args.calc_option == "grad_V":
    ...
    get_avg_s_test(model, valid_loader, train_sampler, args.constraint, weights, r, dataset, seed, sen_attr, t, option, save=True)
elif args.calc_option == "influence":
    get_influence_score(model, train_loader, valid_loader, train_sampler, args.constraint, weights, dataset, seed, sen_attr, t, r, option, load_s_test=True)

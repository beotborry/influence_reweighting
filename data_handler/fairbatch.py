...
class FairBatch(Sampler):
    ...
    """
    def __init__(self, Dataset, batch_size, alpha, target_fairness, replacement = False, seed = 0):
        """Initializes FairBatch."""
        np.random.seed(seed)
        random.seed(seed)
        self.y_data = torch.FloatTensor(Dataset.labels).squeeze()
        self.z_data = torch.FloatTensor(Dataset.groups)
        self.alpha = alpha
        ...
        if target_fairness == 'eqopp' and self.num_labels < 2:
            raise ValueError
        
        # Takes the unique values of the tensors
        self.z_item = list(range(self.num_groups))
        self.y_item = list(range(self.num_labels))
        ...
        self.S_per_label = {}
        for _l in range(self.num_labels):
            tmp = 0
...
#                 print(each_size)
        ...
        for _l in range(self.num_labels):
            for _g in range(self.num_groups):
                key = (_l, _g)
                sort_index[key] = self.select_batch_replacement(each_size[key], self.yz_index[key], self.batch_num, self.replacement)
...
#             sort_index_y_0_z_0 = self.select_batch_replacement(each_size[(0, 0)], self.yz_index[(0,0)], self.batch_num, self.replacement)
        ...
        for i in range(self.batch_num):
            ...
            for _l in range(self.num_labels):
                for _g in range(self.num_groups):
                    key = (_l, _g)                        
#                         batch.append(sort_index[key][i].copy())
                    if batch is None:

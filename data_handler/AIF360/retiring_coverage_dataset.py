...
class RetiringCoverageDataset(StandardDataset):
    """Adult Census Income Dataset.
    See :file:`aif360/data/raw/adult/README.md`.
    """

    def __init__(self, root_dir='./data/retiring_adult_coverage/',
                 label_name='PUBCOV',
        ...
        try:
            df = pd.read_csv(root_dir + '/retiring_adult_coverage.csv', header=0, names=column_names, skipinitialspace=False, na_values=na_values)
            ...
            for col_name in column_names:
                df[col_name] = np.nan_to_num(df[col_name], -1)

            target_transform = lambda x: x == 1
            df['PUBCOV'] = target_transform(df['PUBCOV'])
        ...
        super(RetiringCoverageDataset, self).__init__(df=df, label_name=label_name,
